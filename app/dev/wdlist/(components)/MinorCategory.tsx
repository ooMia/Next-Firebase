...
import { MinorCategoryType } from '@/public/MinorCategoryDummyData'
import { useRouter } from 'next/navigation'
import { useEffect, useState } from 'react'

function MinorCategoryDisplay(setHidden: any, getDataUpdatedBySelected: any, selected: string[]) {
  return (
    <ArrowButton
      className={`flex flex-row-reverse`}
      onClick={() => {
        setHidden((p: boolean) => !p)
      }}
    >
      <div className={`cursor-pointer w-fit flex`} onClick={(event) => event.stopPropagation()}>
        <div className={`text-xl mx-1`}>
          {getDataUpdatedBySelected(selected).map(
            (e: MinorCategoryType) => selected.includes(e.key.toString()) && `${e.name}, `,
          )}
        </div>
      </div>
    </ArrowButton>
...
function MinorCategoryModal(
  hidden: boolean,
  data: MinorCategoryType[],
  selectedByUser: string[],
  setSelectedByUser: any,
) {
  return (
    <div
...
export default function MinorCategory({ majorCategory }: { majorCategory?: number }) {
  ...
  return majorCategory ? (
    <div className={`relative`}>
      {MinorCategoryDisplay(setHidden, getDataUpdatedBySelected, selected)}
      {MinorCategoryModal(hidden, data, selectedByUser, setSelectedByUser)}
    </div>
  ) : (
