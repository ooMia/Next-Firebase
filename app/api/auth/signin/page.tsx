...
const Page = () => {
  ...
  const onSubmitCallback = async (e: any) => {
    ...
    const querySnapshot = await getDocumentsByKeyValue(e, 'users', 'email', userInput)

    if (querySnapshot.empty) {
      await createDocument(e, 'users', obj)
    } else {
      await printQuerySnapshot(querySnapshot)
    }
  }
  ...
  return (
    <div
      className={
        'max-h-full rounded flex flex-col justify-center items-center border-2 bg-white p-4 w-96 m-3'
      }
    >
      ...
      <h2
        className={'text-xs text-center mb-12 tracking-tighter'}
        style={{ color: 'rgb(168,162,168)' }}
      >
        원티드가 제공하는 서비스를
        <br />
      ...
      <h2 className={'text-xs text-gray-600 text-center p-4'}>또는</h2>
      <div className={'flex flex-row justify-center items-center '}>
        ...
        >
          ...
          <h2 className={'text-xs text-gray-600 text-center mb-8 m-2'}>Apple</h2>
        </div>
        <div
          className={'flex flex-col justify-center items-center m-3'}
        ...
        >
          ...
          <h2 className={'text-xs text-gray-600 text-center mb-8 m-2'}>Google</h2>
        </div>
        <div
          className={'flex flex-col justify-center items-center m-3'}
        ...
        >
          ...
          <h2 className={'text-xs text-gray-600 text-center mb-8 m-2'}>Kakao</h2>
        </div>
      </div>
      <h2 className={'text-xs text-gray-800 text-center'}>계정을 잊으셨나요? &gt;</h2>
