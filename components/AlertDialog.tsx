...
export default function AlertDialog() {
  ...
  const handleClose = (e: any, callback: any) => {
  ...
  return (
    <div>
      ...
      >
        <DialogTitle id='alert-dialog-title'>{targetDialog.title}</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          {targetDialog.buttons.map((button) => (
            <Button onClick={(e) => handleClose(e, button.callback)} key={button.name}>
              {button.name}
            </Button>
          ))}
        </DialogActions>
