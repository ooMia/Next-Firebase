    ...
    return (
        <div className={`flex-grow justify-between sticky flex flex-row top-0 left-0 w-full bg-white opacity-90`}>
            <div className={`flex flex-row`}>
                <Link href={"/"}>
                    <Image src={Wanted} alt={"wanted logo"} className={"mx-12 my-3"} priority={true}/>
                </Link>

                <div className={`group w-fit`}>
                    <Button2 key={"채용"} href={"/wdlist"}>채용</Button2>
                    <Button2 key={"커리어"} href={"/wdlist"}>커리어</Button2>
                    <Button2 key={"소셜"} href={"/wdlist"}>소셜</Button2>
                    <Button2 key={"이력서"} href={"/wdlist"}>이력서</Button2>
                    <Button2 key={"프리랜서"} href={"/wdlist"}>프리랜서</Button2>
